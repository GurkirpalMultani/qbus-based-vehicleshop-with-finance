            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        TMKC.Functions.TriggerCallback('tmkc-occasions:server:getVehicles', function(vehicles)
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, TMKC.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Prijs: $'..Config.OccasionSlots[i]["price"]..',-')
                                if Config.OccasionSlots[i]["owner"] == TMKC.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    TMKC.Functions.TriggerCallback('tmkc-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                                ...
                                if IsDisabledControlJustPressed(0, Keys["7"]) then
                                    isConfirming = false
                                    currentVehicle = i
                                    TriggerServerEvent("tmkc-occasions:server:ReturnVehicle", Config.OccasionSlots[i])
                                end
                ...
                if sellDist <= 13.0 and IsPedInAnyVehicle(ped) then 
                    ...
                    if sellDist <= 3.5 and IsPedInAnyVehicle(ped) then
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            TMKC.Functions.TriggerCallback('tmkc-occasions:server:checkVehicleOwner', function(owned)
                                if owned then
                                    openSellContract(true)
                                else
                                    TMKC.Functions.Notify('This vehicle does not belong to you or has outstanding finance payments owed.', 'error', 3500)
                                end
                            end, VehiclePlate)
                        end
                    end
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            TMKC.Functions.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
function openSellContract(bool)
    ...
    SendNUIMessage({
        action = "sellVehicle",
        pData = TMKC.Functions.GetPlayerData(),
...
RegisterNUICallback('error', function(data)
    TMKC.Functions.Notify(data.message, 'error')
end)

RegisterNUICallback('buyVehicle', function()
    local vehData = Config.OccasionSlots[currentVehicle]
    TriggerServerEvent('tmkc-occasions:server:buyVehicle', vehData)
end)

DoScreenFadeIn(250)

RegisterNetEvent('tmkc-occasions:client:BuyFinished')
AddEventHandler('tmkc-occasions:client:BuyFinished', function(model, plate, mods)
    local vehmods = json.decode(mods)

    DoScreenFadeOut(250)
    Citizen.Wait(500)
    TMKC.Functions.SpawnVehicle(model, function(veh)
        SetVehicleNumberPlateText(veh, plate)
        SetEntityHeading(veh, Config.BuyVehicle.h)
        TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
        exports['LegacyFuel']:SetFuel(veh, 100)
        TMKC.Functions.Notify("Vehicle Purchased", "success", 2500)
        TriggerEvent("vehiclekeys:client:SetOwner", plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        TMKC.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('tmkc-occasions:client:ReturnOwnedVehicle')
AddEventHandler('tmkc-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    TMKC.Functions.SpawnVehicle(vehdata.model, function(veh)
        SetVehicleNumberPlateText(veh, vehdata.plate)
        SetEntityHeading(veh, Config.BuyVehicle.h)
        TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
        exports['LegacyFuel']:SetFuel(veh, 100)
        TMKC.Functions.Notify("Your vehicle is returned..")
...
RegisterNUICallback('sellVehicle', function(data)
    local vehicleData = {}
    local PlayerData = TMKC.Functions.GetPlayerData()
    vehicleData.ent = GetVehiclePedIsUsing(GetPlayerPed(-1))
    vehicleData.model = TMKC.Shared.VehicleModels[GetEntityModel(vehicleData.ent)]["model"]
    vehicleData.plate = GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1)))
    vehicleData.mods = TMKC.Functions.GetVehicleProperties(vehicleData.ent)
    vehicleData.desc = data.desc

    TriggerServerEvent('tmkc-occasions:server:sellVehicle', data.price, vehicleData)
...
function sellVehicleWait(price)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    TMKC.Functions.DeleteVehicle(GetVehiclePedIsIn(GetPlayerPed(-1)))
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    TMKC.Functions.Notify('Your car is for sale, $'..price..',-', 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

RegisterNetEvent('tmkc-occasion:client:refreshVehicles')
AddEventHandler('tmkc-occasion:client:refreshVehicles', function()
    if inRange then
        TMKC.Functions.TriggerCallback('tmkc-occasions:server:getVehicles', function(vehicles)
            occasionVehicles = vehicles
            despawnOccasionsVehicles()
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Occasions")
