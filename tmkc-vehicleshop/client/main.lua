...
AddEventHandler('TMKC:Client:OnPlayerLoaded', function()
    TMKC.Functions.GetPlayerData(function(PlayerData)
        ...
        if v["shop"] == "pdm" then
            for cat,_ in pairs(vehicleCategorys) do
                if TMKC.Shared.Vehicles[k]["category"] == cat then
                    table.insert(vehicleCategorys[cat].vehicles, TMKC.Shared.Vehicles[k])
                end
            end
        end
    end
...
RegisterNUICallback('buyVehicle', function(data)
    ...
    TriggerServerEvent('tmkc-vehicleshop:server:buyVehicle', vehicleData, garage)
    openVehicleShop(false)
end)

RegisterNetEvent('tmkc-vehicleshop:client:spawnBoughtVehicle')
AddEventHandler('tmkc-vehicleshop:client:spawnBoughtVehicle', function(vehicle)
