...
function CloseCreator(name, veh, price, financed)
	Citizen.CreateThread(function()
        ...
        TriggerServerEvent('tmkc-vehicleshop:server:setShowroomCarInUse', ClosestVehicle, false)
	end)
end
...
function SelectVehicle(vehicleData)
    TriggerServerEvent('tmkc-vehicleshop:server:setShowroomVehicle', vehicleData, ClosestVehicle)
    close()
end
    ...
    while true do
        ...
        if SellDistance < 20 then
            if IsPedInAnyVehicle(ped) then
                local VehicleData = TMKC.Shared.VehicleModels[GetEntityModel(GetVehiclePedIsIn(ped))]
                if VehicleData["shop"] == "pdm" then
                    ...
                    if SellDistance < 1 then
                        if not SellStarted then
                            ...
                            TMKC.Functions.ShowFloatingHelpNotification("[~g~E~w~] Sell vehicle for: \n".."$"..math.ceil(VehicleData["price"] / 100 * 60),vector3(QB.QuickSell.x, QB.QuickSell.y, QB.QuickSell.z))
                            if IsControlJustPressed(0, Keys["E"]) then
                                SellStarted = true
                            end
                        else
                            --DrawText3Ds(QB.QuickSell.x, QB.QuickSell.y, QB.QuickSell.z, '~b~7~w~ - Confirm / ~b~8~w~ - Cancel')
                            TMKC.Functions.ShowFloatingHelpNotification("[~g~7~w~] Confirm / [~r~8~w~] Cancel",vector3(QB.QuickSell.x, QB.QuickSell.y, QB.QuickSell.z))
                            if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                                SellStarted = false
                                TMKC.Functions.TriggerCallback('tmkc-vehicleshop:server:SellVehicle', function(SoldVehicle)
                                    if SoldVehicle then
                                        TriggerEvent("tmkc-telephone-new:client:BankNotify", "$"..math.ceil(VehicleData["price"] / 100 * 60).." credited")
                                        local veh = GetVehiclePedIsIn(ped)
                                        TMKC.Functions.DeleteVehicle(veh)
                                    else
                                        TMKC.Functions.Notify('This is not your vehicle..', 'error')
                                    end
        ...
        if ClosestVehicle ~= nil then
            if dist < 1.5 then
                if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                    ...
                    local displayName = TMKC.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["name"]
                    local vehPrice = TMKC.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["price"]
                    ...
                    if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                        ...
                        elseif vehshop.opened then
                            if modelLoaded then
                                ...
                                TMKC.Functions.ShowFloatingHelpNotification("You're Choosing a Vehicle",vector3(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65))
                            else
                            	...
                            	TMKC.Functions.ShowFloatingHelpNotification("Vehicle is being picked",vector3(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65))
                            end
                        end
                    else
                    	...
                    	TMKC.Functions.ShowFloatingHelpNotification("Vehicle is in use by another player",vector3(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65))
                    end
                                ...
                                if isValidMenu(vehshop.currentmenu) then
                                    if selected then
                                        if IsControlJustPressed(1, 18) then
                                            if modelLoaded then
                                                TriggerServerEvent('tmkc-vehicleshop:server:setShowroomVehicle', button.model, ClosestVehicle)
                                            end
                                        end
                                    end
                                end
                    ...
                    if IsDisabledControlJustPressed(0, Keys["7"]) then
                        if buySure and not finance then
                            local class = TMKC.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["category"]
                            TriggerServerEvent('tmkc-vehicleshop:server:buyShowroomVehicle', QB.ShowroomVehicles[ClosestVehicle].chosenVehicle, class)
                            buySure = false
                        elseif buySure and finance then
                            local class = TMKC.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["category"]
                            TriggerServerEvent('tmkc-vehicleshop:server:FinanceVehicle',QB.ShowroomVehicles[ClosestVehicle].chosenVehicle, class)
                            buySure = false
                        end
                    end
                    if IsDisabledControlJustPressed(0, Keys["8"]) then
                        TMKC.Functions.Notify('You did not buy the vehicle', 'error', 3500)
                        buySure = false
                    end
...
RegisterNetEvent('tmkc-vehicleshop:client:setShowroomCarInUse')
AddEventHandler('tmkc-vehicleshop:client:setShowroomCarInUse', function(showroomVehicle, inUse)
    QB.ShowroomVehicles[showroomVehicle].inUse = inUse
end)

RegisterNetEvent('tmkc-vehicleshop:client:setShowroomVehicle')
AddEventHandler('tmkc-vehicleshop:client:setShowroomVehicle', function(showroomVehicle, k)
    if QB.ShowroomVehicles[k].chosenVehicle ~= showroomVehicle then
        TMKC.Functions.DeleteVehicle(GetClosestVehicle(QB.ShowroomVehicles[k].coords.x, QB.ShowroomVehicles[k].coords.y, QB.ShowroomVehicles[k].coords.z, 3.0, 0, 70))
        modelLoaded = false
        Wait(250)
...
RegisterNetEvent('tmkc-vehicleshop:client:buyShowroomVehicle')
AddEventHandler('tmkc-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    TMKC.Functions.SpawnVehicle(vehicle, function(veh)
