                ...
                if IsPedInAnyVehicle(ped, false) then
                    if dist < 15 then
                        ...
                        if dist < 2 then
                            ...
                            TMKC.Functions.ShowFloatingHelpNotification("[~g~E~w~] Return the vehicle",vector3(bringcoords.x,bringcoords.y,bringcoords.z))
                            if IsControlJustPressed(0, Keys["E"]) then
                                TMKC.Functions.DeleteVehicle(veh)
                            end
                        end
                    end
                end
...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    for k, v in pairs(TMKC.Shared.Vehicles) do
        if v["shop"] == "custom" then
            for cat,_ in pairs(CustomVehicleCats) do
                if TMKC.Shared.Vehicles[k]["category"] == cat then
                    table.insert(CustomVehicleCats[cat].vehicles, TMKC.Shared.Vehicles[k])
                end
            end
        end
    end
...
Citizen.CreateThread(function()
    ...
    for i = 1, #QBCustom.ShowroomPositions, 1 do
        ...
        if oldVehicle ~= 0 then
            TMKC.Functions.DeleteVehicle(oldVehicle)
        end
            ...
            if dist < 2 then
                if PlayerJob ~= nil then
                    if PlayerJob.name == "cardealer" then
                        ...
                        TMKC.Functions.ShowFloatingHelpNotification("[~b~G~w~] Change Vehicle\n".."/testdrive \n".."/sellv [~b~ID~w~]\n".."/financev [~b~ID~w~]",vector3(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.9))
                                        ...
                                        if selected then
                                            if IsControlJustPressed(1, 18) then
                                                if CustomModelLoaded then
                                                    TriggerServerEvent('tmkc-vehicleshop:server:SetCustomShowroomVeh', button.model, ClosestCustomVehicle)
                                                end
                                            end
                                        end
                                    end
                        ...
                        if ClosestCustomVehicle ~= nil then
                            if QBCustom.ShowroomPositions[ClosestCustomVehicle] ~= nil then
                                if QBCustom.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if TMKC.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        --DrawText3Ds(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6, '~g~7~w~ - Buy / ~r~8~w~ - Cancel - ~g~($'..TMKC.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle].price..',-)')
                                        TMKC.Functions.ShowFloatingHelpNotification("~g~7~w~ - Buy / ~r~8~w~ - Cancel \n".."~g~$"..TMKC.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle].price,vector3(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6))
                                        if IsDisabledControlJustPressed(0, Keys["7"]) then
                                            if not finance then
                                                TriggerServerEvent('tmkc-vehicleshop:server:ConfirmVehicle', QBCustom.ShowroomPositions[ClosestCustomVehicle])
                                                QBCustom.ShowroomPositions[ClosestCustomVehicle].buying = false
                                            elseif finance then
                                                TriggerServerEvent('tmkc-vehicleshop:server:ConfirmVehicleFinance', QBCustom.ShowroomPositions[ClosestCustomVehicle])
                                                QBCustom.ShowroomPositions[ClosestCustomVehicle].buying = false
                                            end
                                        end

                                        if IsDisabledControlJustPressed(0, Keys["8"]) then
                                            TMKC.Functions.Notify('You did NOT buy the car!')
                                            QBCustom.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end
                                    end
                                end
...
RegisterNetEvent('tmkc-vehicleshop:client:SetCustomShowroomVeh')
AddEventHandler('tmkc-vehicleshop:client:SetCustomShowroomVeh', function(showroomVehicle, k)
    CancelEvent()
    if QBCustom.ShowroomPositions[k].vehicle ~= showroomVehicle then
        TMKC.Functions.DeleteVehicle(GetClosestVehicle(QBCustom.ShowroomPositions[k].coords.x, QBCustom.ShowroomPositions[k].coords.y, QBCustom.ShowroomPositions[k].coords.z, 3.0, 0, 70))
...
RegisterNetEvent('tmkc-vehicleshop:client:ConfirmVehicle')
AddEventHandler('tmkc-vehicleshop:client:ConfirmVehicle', function(Showroom, plate)
    TMKC.Functions.SpawnVehicle(Showroom.vehicle, function(veh)
        ...
        TriggerServerEvent("tmkc-customs:server:SaveVehicleProps", TMKC.Functions.GetVehicleProperties(veh))
...
RegisterNetEvent('tmkc-vehicleshop:client:DoTestrit')
AddEventHandler('tmkc-vehicleshop:client:DoTestrit', function(plate)
    if ClosestCustomVehicle ~= 0 then
        TMKC.Functions.SpawnVehicle(QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle, function(veh)
            TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
            exports['LegacyFuel']:SetFuel(veh, 100)
            SetVehicleNumberPlateText(veh, plate)
            SetEntityAsMissionEntity(veh, true, true)
            SetEntityHeading(veh, QBCustom.VehicleBuyLocation.h)
            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(veh))
            TriggerServerEvent("vehicletuning:server:SaveVehicleProps", TMKC.Functions.GetVehicleProperties(veh))
            testritveh = veh
...
RegisterNetEvent('tmkc-vehicleshop:client:SellCustomVehicle')
AddEventHandler('tmkc-vehicleshop:client:SellCustomVehicle', function(TargetId)
    local ped = GetPlayerPed(-1)
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            TriggerServerEvent('tmkc-vehicleshop:server:SellCustomVehicle', TargetId, ClosestCustomVehicle)
        else
            TMKC.Functions.Notify("You are not near the vehicle!", "error")
        end
    else
        TMKC.Functions.Notify("No one around!", "error")
    end
end)

RegisterNetEvent('tmkc-vehicleshop:client:FinanceCustomVehicle')
AddEventHandler('tmkc-vehicleshop:client:FinanceCustomVehicle', function(TargetId)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local player, distance = GetClosestPlayer()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            TriggerServerEvent('tmkc-vehicleshop:server:FinanceCustomVehicle', TargetId, ClosestCustomVehicle)
        else
            TMKC.Functions.Notify("You are not near the vehicle!", "error")
        end
    else
...
function GetClosestPlayer()
    local closestPlayers = TMKC.Functions.GetPlayersFromCoords()
...
RegisterNetEvent('tmkc-vehicleshop:client:SetVehicleBuying')
AddEventHandler('tmkc-vehicleshop:client:SetVehicleBuying', function(slot)
...
RegisterNetEvent('tmkc-vehicleshop:client:SetVehicleFinance')
AddEventHandler('tmkc-vehicleshop:client:SetVehicleFinance', function(slot)
    QBCustom.ShowroomPositions[slot].buying = true
    finance = true
    SetTimeout((60 * 1000) * 5, function()
        QBCustom.ShowroomPositions[slot].buying = false
...
function drawMenuButton(button,x,y,selected)
	...
	if selected then
		...
		SetTextColour(119, 169, 211, 255)
	end
	SetTextCentre(0)
	SetTextEntry("STRING")
	AddTextComponentString(button.name)
	if selected then
		DrawRect(x,y,menu.width,menu.height,98,194,111,150)
	else
		DrawRect(x,y,menu.width,menu.height,0, 0, 0,100)
	end
...
function drawMenuRight(txt,x,y,selected)
	...
	if selected then
		DrawRect(x + menu.width/2 + 0.025, y,menu.width / 3,menu.height,255, 0, 0,100)
	else
		DrawRect(x + menu.width/2 + 0.025, y,menu.width / 3,menu.height,0, 0, 0,150) 
	end
end
